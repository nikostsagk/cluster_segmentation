...
public:
    ...
    explicit ClusterSegmentation(ros::NodeHandle nh) : m_nh(nh)  {
        ...
        if (!nh.getParam("input_cloud_topic", input_cloud_topic)) {
            ROS_ERROR("Could not find 'input_cloud_topic' param in param server.");
            exit(-1);
        }

        // define the subscriber and publisher
        m_sub = m_nh.subscribe(input_cloud_topic, 1, &ClusterSegmentation::cloud_cb, this);
...
void ClusterSegmentation::cloud_cb (const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
{
    ...
    outputPCL.header.frame_id = cloud_msg->header.frame_id;
    pcl_pub_.publish(outputPCL);

    cloudClusters.header.frame_id = cloud_msg->header.frame_id;
